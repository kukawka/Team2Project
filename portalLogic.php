...
$activeCustomerQuery = sprintf("Select Sum(num)from users_per_outlet where end_date = (Select Max(end_date) from totals) order by num desc");
...
$allUsersQuery = sprintf("Select num, outlet_id from users_per_outlet where end_date = (Select Max(end_date) from totals) order by num desc");
